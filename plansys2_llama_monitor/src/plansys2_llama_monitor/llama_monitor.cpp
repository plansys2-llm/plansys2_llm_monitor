...
#include "plansys2_llama_monitor/llama_monitor.hpp"
...
void LLAMAMonitor::configure(
  rclcpp_lifecycle::LifecycleNode::SharedPtr lc_node,
  const std::string & plugin_name)
{
  MonitorBase::configure(lc_node, plugin_name);
...
LLAMAMonitor::create_folders(const std::string & node_namespace)
{
...
std::optional<plansys2_monitor_msgs::msg::Proposal> LLAMAMonitor::propose(
  const std::string & domain, const std::string & problem,
  const std::string & observation,
  const std::string & action_file,
  const std::string & node_namespace,
  const rclcpp::Duration propose_timeout)
{
  cancel_requested_ = false;

  const auto t_propose_start = std::chrono::steady_clock::now();
  ...
  const auto domain_file_path = output_dir / std::filesystem::path("monitor_domain.pddl");
  std::ofstream domain_out(domain_file_path);
  domain_out << domain;
  domain_out.close();

  const auto problem_file_path = output_dir / std::filesystem::path("monitor_problem.pddl");
  std::ofstream problem_out(problem_file_path);
  problem_out << problem;
  problem_out.close();

  const auto observation_file_path = output_dir / std::filesystem::path("monitor_observation.pddl");
  std::ofstream observation_out(observation_file_path);
  observation_out << observation;
  observation_out.close();


  const auto resolution_file_path = output_dir / std::filesystem::path("monitor_resolution.txt");
  ...
  RCLCPP_INFO(logger,
    "[llama-monitor] Starting propose (timeout=%.0fs, summarize=%s, llm_debug=%s, "
    "prompt_debug=%s, pre_launch=%s)",
    propose_timeout.seconds(),
  ...
  if (!pre_launch_) {
    ...
    RCLCPP_WARN(logger,
      "[llama-monitor] pre_launch=true but server is not running; "
  ...
  const auto prompt_file_path = output_dir / std::filesystem::path("monitor_prompt.txt");
  ...
  // Only tear down the LLM server in per-propose mode. In pre_launch mode the
  ...
  const auto elapsed_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
    std::chrono::steady_clock::now() - t_propose_start).count();

  if (cancelled) {
    RCLCPP_WARN(logger, "[llama-monitor] Cancelled mid-propose after %ld ms", elapsed_ms);
    return std::nullopt;
  }

  RCLCPP_INFO(logger, "[llama-monitor] propose() completed in %ld ms (%zu chars response)",
...
void LLAMAMonitor::initialize(const std::string & node_name)
{
  std::cout << "Initializing monitor with node: " << node_name << std::endl;
}

void LLAMAMonitor::launch_llm_server()
{
  if (launch_pid_ != -1) {
    return;
  }
...
void LLAMAMonitor::shutdown_llm_server()
{
  if (launch_pid_ == -1) {
    return;
  }
  ...
  // lifecycle node is already being torn down. The propose() path prints its own
...
PLUGINLIB_EXPORT_CLASS(plansys2::LLAMAMonitor, plansys2::MonitorBase);
