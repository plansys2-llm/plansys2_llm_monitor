...
#ifndef PLANSYS2_LLAMA_MONITOR__LLAMA_MONITOR_HPP_
#define PLANSYS2_LLAMA_MONITOR__LLAMA_MONITOR_HPP_
...
/**
 * @class plansys2::LLAMAMonitor
 * @brief Monitor plugin that uses a local LLM (via llama_ros) as a reasoner over PDDL state.
 *
 * Implements the MonitorBase interface. Forks `ros2 llama launch` to start the
 * llama_node server and `ros2 llama prompt` to send prompts, capturing the JSON
 * reply and filling a plansys2_monitor_msgs::msg::Proposal with the classification
 ...
 * backends (local models, paid APIs) without pulling their SDKs into the monitor
 * process. When pre_launch=true the server is started once at configure() and
 * kept alive for the plugin's lifetime.
 */
class LLAMAMonitor : public MonitorBase
{
public:
  /**
   ...
   */
  ~LLAMAMonitor() override;

  /**
   * @brief Configures the monitor plugin and snapshots its parameters.
   *
   ...
   * llama_node server so the first propose() does not pay the model-load cost.
   *
  ...
  std::optional<plansys2_monitor_msgs::msg::Proposal> propose(
    const std::string & domain, const std::string & problem,
    const std::string & observation,
    const std::string & action_file,
    const std::string & node_namespace = "",
    const rclcpp::Duration propose_timeout = 15s) override;
...
#endif  // PLANSYS2_LLAMA_MONITOR__LLAMA_MONITOR_HPP_
