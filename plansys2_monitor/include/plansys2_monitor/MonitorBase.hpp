...
#ifndef PLANSYS2_MONITOR__MONITORBASE_HPP_
#define PLANSYS2_MONITOR__MONITORBASE_HPP_
...
#include "plansys2_monitor_msgs/msg/proposal.hpp"
...
{
public:
  using Ptr = std::shared_ptr<plansys2::MonitorBase>;
  MonitorBase() = default;
  ...
  virtual std::optional<plansys2_monitor_msgs::msg::Proposal> propose(
    const std::string & domain, const std::string & problem,
    const std::string & observation,
    const std::string & action_file,
    const std::string & node_namespace = "",
    const rclcpp::Duration propose_timeout = 120s) = 0;
...
protected:
  ...
  static inline plansys2_monitor_msgs::msg::Proposal parseResponse(const std::string & raw_response)
  {
    plansys2_monitor_msgs::msg::Proposal proposal;
    proposal.resolution = raw_response;
    proposal.classification = plansys2_monitor_msgs::msg::Proposal::ERROR;
    ...
    try {
      ...
      if (classification_str == "CORRECT") {
        proposal.classification = plansys2_monitor_msgs::msg::Proposal::CORRECT;
      } else if (classification_str == "MODIFY_PLAN") {
        proposal.classification = plansys2_monitor_msgs::msg::Proposal::MODIFY_PLAN;
      } else if (classification_str == "MODIFY_DOMAIN") {
        proposal.classification = plansys2_monitor_msgs::msg::Proposal::MODIFY_DOMAIN;
      } else if (classification_str == "UNSOLVABLE") {
        proposal.classification = plansys2_monitor_msgs::msg::Proposal::UNSOLVABLE;
      } else {
        return proposal;
      }
      ...
      if (j.contains("remove_predicates")) {
        appendStrings(j["remove_predicates"], proposal.remove_predicates);
      }
      if (j.contains("add_predicates")) {
        appendStrings(j["add_predicates"], proposal.add_predicates);
      }
      if (j.contains("add_instances")) {
        appendStrings(j["add_instances"], proposal.add_instances);
      }
      if (j.contains("domain_changes")) {
        appendStrings(j["domain_changes"], proposal.domain_changes);
      }
...
#endif  // PLANSYS2_MONITOR__MONITORBASE_HPP_
